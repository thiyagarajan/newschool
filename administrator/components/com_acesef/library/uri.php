...
class AcesefURI {
	...
	function getPageNumber($vars, $ext_params, $item_limitstart = false) {
		...
		if ($option == 'com_content') {
			...
			$menu_item = AcesefUtility::getMenu()->getItem(intval($Itemid));
	...
	function fixPaginationURI(&$uri) {
		...
		if (!empty($Itemid)) {
			$menu = AcesefUtility::getMenu()->getItem($Itemid);
    ...
    function getDefaultMenuItem() {
        ...
        // Workaround, the Joomla one is bugged
	...
	function _checkDB(&$uri, $prev_lang) {
        ...
        if (AcesefUtility::getConfigState($this->attributes->params, 'ignore_multi_itemid')) {
		    $url_found = AcesefCache::checkURL($real_url, false, false, true);
        }
        else {
    ...
    function sendHeader($header) {
		...
		elseif ($this->AcesefConfig->sent_headers_error == 1) {
