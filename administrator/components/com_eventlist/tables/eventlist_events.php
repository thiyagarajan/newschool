...
class eventlist_events extends JTable
{
	...
	function check($elsettings)
	{
		...
		if ( $this->title == '' ) {
			$this->_error = JText::_( 'COM_EVENTLIST_ADD_TITLE' );
      		JError::raiseWarning('SOME_ERROR_CODE', $this->_error );
      		return false;
		}

		if ( $titlelength > 100 ) {
      		$this->_error = JText::_( 'COM_EVENTLIST_ERROR_TITLE_LONG' );
      		JError::raiseWarning('SOME_ERROR_CODE', $this->_error );
      		return false;
		}
		...
		if (!preg_match("/^[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]$/", $this->dates)) {
	      	$this->_error = JText::_( 'COM_EVENTLIST_DATE_WRONG' );
	      	JError::raiseWarning('SOME_ERROR_CODE', $this->_error );
	      	return false;
		}
		...
		if (isset($this->recurrence_counter)) {
			if (!preg_match("/^[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]$/", $this->recurrence_counter) and $this->recurrence_counter <> 'UNLIMITED') {
	 		     	$this->_error = JText::_( 'COM_EVENTLIST_DATE_WRONG' );
	 		     	JError::raiseWarning('SOME_ERROR_CODE', $this->_error );
	 		     	return false;
			}
		...
		if (isset($this->times)) {
   			if (!preg_match("/^[0-2][0-9]:[0-5][0-9]$/", $this->times)) {
      			$this->_error = JText::_( 'COM_EVENTLIST_TIME_WRONG' );
      			JError::raiseWarning('SOME_ERROR_CODE', $this->_error );
      			return false;
	  		}
		}
		...
		if($this->locid == '') {
			$this->_error = JText::_( 'COM_EVENTLIST_VENUE_EMPTY');
			JError::raiseWarning('SOME_ERROR_CODE', $this->_error );
			return false;
		}

		if($this->catsid == 0) {
			$this->_error = JText::_( 'COM_EVENTLIST_CATEGORY_EMPTY');
			JError::raiseWarning('SOME_ERROR_CODE', $this->_error );
			return false;
		}
