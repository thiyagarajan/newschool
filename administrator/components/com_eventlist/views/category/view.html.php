...
class EventListViewCategory extends JView {
	...
	function display($tpl = null)
	{
        ...
        $mediapath = JURI::root().$mediaparams->get($path, 'images');
		$javascript = "onchange=\"javascript:if (document.forms[0].image.options[selectedIndex].value!='') {document.imagelib.src='" . $mediapath . "' + '/eventlist/categories/' + document.forms[0].image.options[selectedIndex].value} else {document.imagelib.src='../media/system/images/blank.png'}\"";
		$Lists['imagelist'] 		= JHTML::_('list.images', 'image', $row->image, $javascript, $mediaparams->get($path, 'images').'/eventlist/categories/' );
		$Lists['access'] 			= JHTML::_('access.assetgrouplist', 'access', 1);
