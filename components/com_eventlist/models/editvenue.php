...
class EventListModelEditvenue extends JModel
{
    ...
    $text_description = JFilterOutput::cleanText($row->locdescription);
		...
		if (($elsettings->mailinform == 2) || ($elsettings->mailinform == 3)) {
			...
			$state 	= $row->published ? JText::sprintf('COM_EVENTLIST_MAIL_VENUE_PUBLISHED', $link) : JText::_('COM_EVENTLIST_MAIL_VENUE_UNPUBLISHED');

			If ($edited) {

				$modified_ip 	= getenv('REMOTE_ADDR');
				$edited 		= JHTML::Date( $row->modified, JText::_( 'DATE_FORMAT_LC2' ) );
				$mailbody 		= JText::sprintf('COM_EVENTLIST_MAIL_EDIT_VENUE', $user->name, $user->username, $user->email, $modified_ip, $edited, $row->venue, $row->url, $row->street, $row->plz, $row->city, $row->country, $text_description, $state);
				$mail->setSubject( $SiteName.JText::_( 'COM_EVENTLIST_EDIT_VENUE_MAIL' ) );

			} else {

				$created 		= JHTML::Date( $row->modified, JText::_( 'DATE_FORMAT_LC2' ) );
				$mailbody 		= JText::sprintf('COM_EVENTLIST_MAIL_NEW_VENUE', $user->name, $user->username, $user->email, $row->author_ip, $created, $row->venue, $row->url, $row->street, $row->plz, $row->city, $row->country, $text_description, $state);
				$mail->setSubject( $SiteName.JText::_( 'COM_EVENTLIST_NEW_VENUE_MAIL' ) );
		...
		if (($elsettings->mailinformuser == 2) || ($elsettings->mailinformuser == 3)) {
			...
			$state 	= $row->published ? JText::sprintf('COM_EVENTLIST_USER_MAIL_VENUE_PUBLISHED', $link) : JText::_('COM_EVENTLIST_USER_MAIL_VENUE_UNPUBLISHED');

			If ($edited) {

				$edited 		= JHTML::Date( $row->modified, JText::_( 'DATE_FORMAT_LC2' ) );
				$mailbody 		= JText::sprintf('COM_EVENTLIST_USER_MAIL_EDIT_VENUE', $user->name, $user->username, $edited, $row->venue, $row->url, $row->street, $row->plz, $row->city, $row->country, $text_description, $state);
				$usermail->setSubject( $SiteName.JText::_( 'EDIT USER VENUE MAIL' ) );

			} else {

				$created 		= JHTML::Date( $row->modified, JText::_( 'DATE_FORMAT_LC2' ) );
				$mailbody 		= JText::sprintf('COM_EVENTLIST_USER_MAIL_NEW_VENUE', $user->name, $user->username, $created, $row->venue, $row->url, $row->street, $row->plz, $row->city, $row->country, $text_description, $state);
