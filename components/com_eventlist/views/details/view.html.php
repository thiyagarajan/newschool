...
class EventListViewDetails extends JView
{
	...
	function display($tpl = null)
	{
		...
		$pathway->addItem( JText::_( 'COM_EVENTLIST_DETAILS' ). ' - '.$row->title, JRoute::_('index.php?view=details&id='.$row->slug));
		...
		if (($row->locdescription == '') || ($row->locdescription == '<br />')) {
			$row->locdescription = JText::_( 'COM_EVENTLIST_NO_DESCRIPTION' );
		} else {
			//execute plugins
		...
		if (!$item){
            ...
            $document->setTitle( $item->title.' - '.$row->title );
        }
