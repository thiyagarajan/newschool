...
class EventListViewVenueevents extends JView
{
	...
	function display( $tpl = null )
	{
		...
		//create the pathway
		if ($task == 'archive') {
			$pathway->addItem( JText::_( 'COM_EVENTLIST_ARCHIVE' ).' - '.$venue->venue, JRoute::_('index.php?option='.$option.'&view=venueevents&task=archive&id='.$venue->slug));
    ...
    $allowedtoeditvenue = ELUser::editaccess($elsettings->venueowner, $venue->created_by, $elsettings->venueeditrec, $elsettings->venueedit);
		...
		if (empty ($venue->locdescription)) {
			$venuedescription = JText::_( 'COM_EVENTLIST_NO_DESCRIPTION' );
		} else {
			//execute plugins
