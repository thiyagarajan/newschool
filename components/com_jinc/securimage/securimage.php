...
//function
    ...
    function saveData() {
        ...
        $session->set($this->session_prefix . '.securimage_code_value', strtolower($this->code));
        $session->set($this->session_prefix . '.securimage_code_ctime', time());
    ...
    function validate() {
        ...
        $session = JFactory::getSession();
        $code_value = $session->get($this->session_prefix . '.securimage_code_value', '');
        if (trim($code_value) != '') {
            $code_time = $session->get($this->session_prefix . '.securimage_code_ctime');
            if ($this->isCodeExpired($code_time) == false) {
                $code = $code_value;
            }
        ...
        if ($code != '') {
            if ($code == $code_entered) {
                ...
                // $session = JFactory::getSession();
                // $session->set($this->session_prefix . '.securimage_code_value', '');
                // $session->set($this->session_prefix . '.securimage_code_ctime', '');
    ...
    function saveCodeToDatabase() {
        ...
        if ($this->use_sqlite_db && $this->sqlite_handle !== false) {
            ...
            $session = JFactory::getSession();
            $code_value = $session->get($this->session_prefix . '.securimage_code_value');
            $code = $code_value; // hash code for security - if cookies are disabled the session still exists at this point
