...
class plgSystemArticlesAnywhereHelper
{
	...
	function processArticle($id, $art, $text = '', $type = 'article', $first = 0)
	{
		if (!$first) {
			// do second pass
			$text = $this->processArticle($id, $art, $text, $type, 1);
		}
		...
		$where = ' AND ( a.title = '.$this->database->quote(NNFrameworkFunctions::html_entity_decoder($id));
		$where .= ' OR a.alias = '.$this->database->quote(NNFrameworkFunctions::html_entity_decoder($id));
		if (is_numeric($id)) {
			$where .= ' OR a.id = '.$id;
		}
		$where .= ' ) ';
